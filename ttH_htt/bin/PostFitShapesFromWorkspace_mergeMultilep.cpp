...
int main(int argc, char* argv[]) {
  ...
  vector<vector<string>> combined_bins;
  vector<string> bins_2lss_0tau = {
    "ttH_2lss_ee_neg", "ttH_2lss_ee_pos",
    "ttH_2lss_em_bl_neg", "ttH_2lss_em_bl_pos", "ttH_2lss_mm_bl_neg", "ttH_2lss_mm_bl_pos",
    "ttH_2lss_em_bt_neg", "ttH_2lss_em_bt_pos", "ttH_2lss_mm_bt_neg", "ttH_2lss_mm_bt_pos"};
  vector<string> bins_3l_0tau = {
    "ttH_3l_bl_neg", "ttH_3l_bl_pos",
    "ttH_3l_bt_neg", "ttH_3l_bt_pos"};
  vector<string> bins_2lss_0tau_3j = {
      "ttH_2lss_ee_neg_3j", "ttH_2lss_ee_pos_3j",
      "ttH_2lss_em_bl_neg_3j", "ttH_2lss_em_bl_pos_3j", "ttH_2lss_mm_bl_neg_3j", "ttH_2lss_mm_bl_pos_3j",
      "ttH_2lss_em_bt_neg_3j", "ttH_2lss_em_bt_pos_3j", "ttH_2lss_mm_bt_neg_3j", "ttH_2lss_mm_bt_pos_3j"};
  vector<string> bins_3l_0tau_zpeak = {
    "ttH_3l_bl_neg_zpeak", "ttH_3l_bl_pos_zpeak",
    "ttH_3l_bt_neg_zpeak", "ttH_3l_bt_pos_zpeak"};
  //vector<string> bins_4l_0tau = {"ttH_4l"};

  combined_bins.push_back(bins_2lss_0tau);
  combined_bins.push_back(bins_3l_0tau);
  combined_bins.push_back(bins_2lss_0tau_3j);
  combined_bins.push_back(bins_3l_0tau_zpeak);
  //combined_bins.push_back(bins_4l_0tau);

  vector<string> name_bins;
  name_bins.push_back("ttH_2lss_0tau");
  name_bins.push_back("ttH_3l_0tau");
  name_bins.push_back("ttH_2lss_0tau_3j");
  name_bins.push_back("ttH_3l_0tau_zpeak");
  ...
  if (!skip_prefit) {
    //for (auto bin : bins) {
    for(unsigned int i_bin=0; i_bin<name_bins.size(); i_bin++){
      //ch::CombineHarvester cmb_bin = cmb.cp().bin({bin});
      string bin = name_bins[i_bin];
      vector<string> bins = combined_bins[i_bin];
      ch::CombineHarvester cmb_bin = cmb.cp().bin(bins);
      ...
      if (datacard != "") {
        ...
        TH1F ref = cmb_card.cp().bin(bins).GetObservedShape();
  ...
  if (postfit) {
    ...
    for(unsigned int i_bin=0; i_bin<name_bins.size(); i_bin++){
      ...
      for (auto proc : cmb_bin.process_set()) {
        ...
        //string proc = name_procs[i_proc];
        //vector<string> procs = combined_procs[i_proc];
    ...
    if (factors) {
      ...
      for(unsigned int i_bin=0; i_bin<name_bins.size(); i_bin++){
        ...
        ch::CombineHarvester cmb_bkgs = cmb.cp().bin(bins).backgrounds();
