...
def BuildScan(scan, param, files, color, yvals, ycut):
    ...
    bestfit = graph.GetY()[0]
    bestfitY = ROOT.TMath.MaxElement(graph.GetN(),graph.GetY()) #graph.GetY()[0]
    print bestfitY
    for i in xrange(graph.GetN()):
        if (bestfitY > graph.GetY()[i]) : ## Xanda
            print (i, bestfit, bestfitY)
            bestfit = graph.GetX()[i]
            bestfitY = graph.GetY()[i]
...
procsRate = open(args.POI+"_rate.txt", 'w')
...
print (fixed_name, "Observed: ")
print (val_nom[0])
print ( val_nom[1])
print ( abs(val_nom[2]))
textfit = '%s = %.3f{}^{#plus %.3f}_{#minus %.3f}' % (fixed_name, val_nom[0], val_nom[1], abs(val_nom[2]))
procsRate.write("Observed: "+str(val_nom[0])+" "+str(val_nom[1])+" -"+str(abs(val_nom[2]))+"\n")
...
if args.breakdown is None:
    for i, other in enumerate(other_scans):
        ...
        pt.AddText(textfit)
        print (fixed_name, "Expected: ", other['val'][0], other['val'][1], abs(other['val'][2]))
        procsRate.write("Expected: "+str(other['val'][0])+" "+str(other['val'][1])+" -"+str(abs(other['val'][2]))+"\n")
procsRate.close()
print ("rates saves on: "+args.POI+"_rate.txt")
